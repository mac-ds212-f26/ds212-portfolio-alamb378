...
> The goal of NYT's was to represtent the claims of the scince and show the readers in a more personal way how global warming effects them.
> This informs the layers of the graph letting reads put in they're own cities and how it compares to record highs/lows and the normal range across the year.
...
```{r }
monthly_lines <- seq(as.Date("2011-01-01"), as.Date("2011-12-31"), by = "1 month")

day_lines <- as.numeric(format(monthly_lines, "%j"))
...
weather |> 
  ...
  geom_linerange(aes(x=dateInYear, ymin = NormalLow, ymax = NormalHigh), color="#C8B8BA")+
  geom_linerange(aes(x=dateInYear, ymin = Low, ymax = High), color="#A90248")+
  geom_vline(xintercept = day_lines, linetype = "dashed", color = "#ECEBE3")+
  theme_classic()
...
month_rep <- data.frame(
  monthly_lines = month.name, day_lines = day_lines)
...
```{r weather_plot_precip}shape = RecordP}
# Get a subset with just the days that had a recod high precip
Record_weather <- weather |> 
  filter(RecordP == TRUE)
# Make a list of the last days in the month
month_starts <- seq(ymd("2026-01-01"), ymd("2026-12-01"), by = "1 month")

# 2. Find the last date of each month
month_ends <- ceiling_date(month_starts, unit = "month") - days(1)

# 3. Get the day of the year (1–365 or 366) for each month-end
last_days_of_year <- yday(month_ends)

tot_prec <- weather |> 
  filter(dateInYear %in% last_days_of_year)
  


ggplot()+
  geom_area(data = weather, aes(x=dateInYear, y=CulmPrec, group = Month),colour = "#32a3d8", fill = "#ebeae2")+
  geom_point(data = Record_weather, aes(x=dateInYear, y=CulmPrec, shape = RecordP), shape = 17)+
  geom_text(data = tot_prec,  aes(x=dateInYear, y=CulmPrec, label = CulmPrec), hjust = 1, vjust = -0.25, size=2)+
  theme_classic()
